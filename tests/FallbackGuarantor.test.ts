...
it('Fallback guarantor accept operation after deadline', async () => {
    ...
    const executionResult = await contract.sendInternalMessage(new InternalMessage({
        to: getAddress(contract),
        from: defaultInitialDataConfig.guarantors.guarantorFallback,
        value: 20_000_000,
        bounce: false,
    ...
    expect(sendMsgAction2Destination.equals(defaultInitialDataConfig.guarantors.guarantorFallback)).toBeTruthy()
}, timeout)

it('Fallback guarantor reject operation after deadline', async () => {
    const config: InitialDataConfig = {...defaultInitialDataConfig}
    config.guarantors.deadline = 0
    const contract: SmartContract = await create(config)
    await contract.sendInternalMessage(new InternalMessage({
...
it('Fallback guarantor invalid operation - initialization. Error: UnknownOperation', async () => {
    const config: InitialDataConfig = {...defaultInitialDataConfig}
    config.guarantors.deadline = 0
    const contract: SmartContract = await create(config)
    await contract.sendInternalMessage(new InternalMessage({
...
it('Fallback guarantor operation after fallback deadline. Error: InvalidSender', async () => {
    const config: InitialDataConfig = {...defaultInitialDataConfig}
    config.guarantors.deadline = 0
    config.guarantors.deadlineFallback = 0
    const contract: SmartContract = await create(config)
    await contract.sendInternalMessage(new InternalMessage({
        to: getAddress(contract),
        value: 20_000_000,
