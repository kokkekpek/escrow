...
it('Seller accept operation after deadline', async () => {
    ...
    const executionResult = await contract.sendInternalMessage(new InternalMessage({
        to: getAddress(contract),
        from: defaultInitialDataConfig.seller,
        value: 20_000_000,
        bounce: false,
    ...
    expect(sendMsgAction2Destination.equals(defaultInitialDataConfig.seller)).toBeTruthy()
}, timeout)

it('Seller reject operation after deadline', async () => {
    const config: InitialDataConfig = {...defaultInitialDataConfig}
    config.guarantors.deadline = 0
    config.guarantors.deadlineFallback = 0
    const contract: SmartContract = await create(config)
...
it('Seller invalid operation - initialization. Error: UnknownOperation', async () => {
    const config: InitialDataConfig = {...defaultInitialDataConfig}
    config.guarantors.deadline = 0
    config.guarantors.deadlineFallback = 0
    const contract: SmartContract = await create(config)
